...
interface EventStatsResult {
  baseStats: BaseStats;
  typeStats: TypeSpecificStats;
  audienceInsights: AudienceInsight[];
  kpiCharts: KpiChartData[];
  eventType: string;
}

const DEFAULT_BASE_STATS: BaseStats = {
  tickets: 0, sessions: 0, speakers: 0, exhibitors: 0,
  forms: 0, marketing: 0, registrations: 0, revenue: 0,
  ticketsSold: 0, ticketsTotal: 0, avgPrice: 0, currency: 'TND'
};

async function fetchEventStats(eventId: string): Promise<EventStatsResult> {
  ...
  // 2. Base Queries (Shared across all events) — all in parallel
  const [
    ticketsCount,
    sessionsCount,
    speakersCount,
  ...
  let sold = 0, total = 0, revenue = 0, currency = 'TND', avgPrice = 0;
  ...
  if (['summit', 'conference'].includes(type.toLowerCase())) {
    ...
    typeStats.sponsorsCount = sponsorsCount || 0;
    typeStats.abstractsCount = 0;
    ...
    typeStats.certificatesIssued = (certificatesCount as any)?.count || 0;
    typeStats.avgQuizScore = 0;
  } else if (type.toLowerCase() === 'networking') {
    typeStats.connectionsMade = meetingsCount || 0;
  }
  ...
  return { baseStats, typeStats, audienceInsights: insights, kpiCharts: charts, eventType: type };
}
...
export function useEventStats(eventId?: string) {
  ...
  return {
    baseStats: data?.baseStats || DEFAULT_BASE_STATS,
    typeStats: data?.typeStats || {},
    audienceInsights: data?.audienceInsights || [],
    kpiCharts: data?.kpiCharts || [],
